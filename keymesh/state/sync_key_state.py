"""
SyncKeyState — Thread-safe runtime state for a single API key in a synchronous context.

Tracks: active requests, cooldown, success/failure counts, latency, and last-use timestamp.
All mutations are protected by a threading.Lock for thread safety.
"""
...
class SyncKeyState:
    ...
    acquire the internal threading lock — never mutate fields directly.
    """
    ...
    # ── Mutation helpers (all synchronous, lock-protected) ───────────────────────
    ...
    def apply_cooldown(self, duration: float = 60.0) -> None:
        """
    ...
    def __repr__(self) -> str:
        return (
            f"SyncKeyState(key=...{self.key[-6:]!r}, "
